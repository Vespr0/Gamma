...
function BaseAbility.new(name: string, entity: TypeEntity.BaseEntity, tool, abilityConfig)
	...
	if not abilityConfig then
		error("Ability configuration is missing")
		return
	end
...
function BaseAbility:getCurrentFakeTool()
	...
	if not self.entity.rig then
		warn("Entity rig not found")
		return nil
	end
...
function BaseAbility:getCurrentFakeToolHandle()
	...
	if not tool then
		warn("Fake tool not found")
		return nil
	end
	if not tool.Model then
		warn("Fake tool model not found")
		return nil
	end
...
function BaseAbility:getCurrentFakeToolMuzzlePosition()
	...
	if not handle then
		warn("Fake tool handle not found")
		return nil
	end
...
function BaseAbility:readAction(func)
	if IS_SERVER then
		self.trove:Add(
			AbilitiesMiddleware.ReadAbility:Connect(
				function(entityID: number, abilityName: string, toolIndex: number, ...)
					...
					if toolIndex ~= self.tool:GetAttribute("Index") then
						return
					end
...
function BaseAbility:sendAction(...)
	if IS_SERVER then
		...
		return AbilitiesMiddleware.SendAbility:Fire(
			replicationPlayer,
			self.entity.id,
			self.name,
			self.tool:GetAttribute("Index"),
			select(2, ...)
		)
	else
