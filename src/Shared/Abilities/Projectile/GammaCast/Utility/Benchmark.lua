...
function Utility.Benchmark(Label: string, Samples: number, Closure: (Begin: (Name: string) -> (), End: (Name: string) -> (), ...any) -> (...any), ...: any)
	...
	do
		...
		`Fastest Time: {Format(Results[1])}`)
		`Average Time: {Format(Elapsed / Samples)}`)
		`50th Percentile: {Format(Results[Index50th])}`)
		`95th Percentile: {Format(Results[Index95th])}`)
		`99th Percentile: {Format(Results[Index99th])}`)
	end
	...
	for Name, Tag in Tags do
		...
		warn(`TAG: {Name}`)
		`	Sample Time: {Format(Times[Index50th] * Invocations[InvocationsIndex50th])}`)
		`	Fastest Time: {Format(Times[1])}`)
		`	Average Time: {Format(Elapsed / #Times)}`)
		`	Average Invocations: {Samples / #Invocations}`)
		`	50th Percentile: {Format(Times[Index50th])}`)
		`	95th Percentile: {Format(Times[Index95th])}`)
		`	99th Percentile: {Format(Times[Index99th])}`)
